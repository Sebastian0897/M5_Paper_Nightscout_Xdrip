...
void CleanTop() // Clean display from possible artifacts
{
 canvas.deleteCanvas();  
...
Serial.println(now());
Serial.println("Time when to update");
Serial.println(bz_time_alt_in_five);
while (bz_time_alt_in_five - now() <=30 && bz_time_alt_in_five - now() > 0 )
{
  delay(900);
  Serial.println("waiting for the right time");
}
if (bz_time_alt_in_five <= now() || RTCtime.min == 5)
{
ConnectWifi();
delay(100);
if(WiFi.status()== WL_CONNECTED){    
...
int root_0_delta = root_0["delta"]; // -0.996
...
if (bz_time_alt != BZunixtime && BZunixtime != 0 || RTCtime.min == 5) // the display is only updates when a new value exist. Determined by the timestamp. The timestamp is in Unix time. and as a failure catch is !=0
{
canvas.deleteCanvas();  
canvas.createCanvas(330,60);// Size requiered for the display
...
char D[1];
dtostrf(root_0_delta,2,0,D);// convert to the correct file format for display purposes
if (root_0_delta >= 0)
{
...
else if (root_0_delta >= 5 && root_0_delta < 10 )
{
...
char v[2];
M5.BatteryADCBegin();
float percent = ((M5.getBatteryVoltage()-3400)/0.85)*100;
...
M5.RTC.getTime(&RTCtime);
int timer; 
timer = 56- RTCtime.sec;
M5.shutdown(timer);// turns of the device, RTC starts the Device again in X-secs. should adjust sleep lenght automatically as long as the task does not take over 1min to complete. has a safety margin of 4 secs. about 2,5 seconds are actually required to boot
}
...
void loop() { // the actuall part with determines what runs when. some thing dont need to run every minute. Since it is a e-paper display the information will be retained.  
  ...
  M5.RTC.getTime(&RTCtime);
  if (RTCtime.sec == 0) // to loop every minute
  {
...
{  
 ...
 delay(400);  // so it does not try the first if-loop so fast and often. energy saving, I think?
}
}
